...
const onlyWatchInput = document.querySelector("#only-watch-streamer-name-input")
...
const disableBlurContainer = document.querySelector("#disable-streamers")
const enableBlurContainer = document.querySelector("#enable-streamers")
...
chrome.storage.sync.get([STORAGE_RADIO_KEY], function(result) {
    ...
    if (mode == null) {
        ...
        disableRadioClicked()
        blurContainer(enableBlurContainer)
    } else if (mode === "disabled"){
        blurContainer(enableBlurContainer)
        disableRadio.checked = true
    } else if (mode === "only-watch"){
        blurContainer(disableBlurContainer)
...
function blurContainer(container) {
    container.style.setProperty("-webkit-filter", `blur(${BLUR})`)
    container.style.setProperty("-moz-filter", `blur(${BLUR})`)
    container.style.setProperty("-o-filter", `blur(${BLUR})`)
    container.style.setProperty("-ms-filter", `blur(${BLUR})`)
    container.style.setProperty("filter", `blur(${BLUR})`)
}

function removeContainerBlur(container) {
...
document.body.addEventListener('change', e => {
    ...
    switch (e.target) {
        case disableRadio:
            ...
            blurContainer(enableBlurContainer)
            removeContainerBlur(disableBlurContainer)
            break
        case onlyWatchRadio:
            setItem(STORAGE_RADIO_KEY, "only-watch")
            removeContainerBlur(enableBlurContainer)
            blurContainer(disableBlurContainer)
            break
        case noModeRadio:
            removeContainerBlur(enableBlurContainer)
